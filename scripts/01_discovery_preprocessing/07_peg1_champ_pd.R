...
save(myLoadPD,file = 'GSE111629_champ_myLoadPD330.Rdata')
load(file = 'GSE111629_champ_myLoadPD330.Rdata')
...
champ.QC(beta = myLoadPD$beta,
         ...
         Rplot=TRUE,
         Feature.sel="None",
         resultsDir="./CHAMP_QCimages_330PD/")

myNormPD <- champ.norm(beta=myLoadPD$beta,
                       rgSet=myLoadPD$rgSet,
                       mset=myLoadPD$mset,
                       resultsDir="./CHAMP_Normalization_330PD/",
                       method="BMIQ",
                       plotBMIQ=FALSE,
                       arraytype="450K",
                       cores=3)
class(myNormPD)   #"matrix" "array" 
dim(myNormPD)  #[1] 400053    330
pD=myLoadPD$pd
save(myNormPD,pD,file = 'GSE111629_champ_myNormPD330.Rdata')
...
champ.SVD(beta = myCombatPD %>% as.data.frame(),
          ...
          PDFplot=TRUE,
          Rplot=TRUE,
          resultsDir="./CHAMP_SVDimages_330PD_AftercombatSlide/")
...
myCombatPD <- champ.runCombat(beta=myCombatPD,
                              ...
                              batchname=c("Array"),
                              logitTrans=TRUE)
save(myCombatPD,pD,file = 'GSE111629_champ_myCombatPD330.Rdata')
load(file = 'GSE111629_champ_myCombatPD330.Rdata')
...
champ.SVD(beta = myCombatPD %>% as.data.frame(),
          ...
          PDFplot=TRUE,
          Rplot=TRUE,
          resultsDir="./CHAMP_SVDimages_330PD_AftercombatSlideArray/")
...
save(myRefbasePD,file = 'GSE111629_champ_myRefbasePD330.Rdata')
load(file = 'GSE111629_champ_myRefbasePD330.Rdata')


?champ.DMP
myDMPPD0.05 <- champ.DMP(beta = myRefbasePD$CorrectedBeta,
                     pheno = pD$Sex,
                     compare.group = NULL,
                     adjPVal = 0.05,
                     adjust.method = "BH",
                     arraytype = "450K")
head(myDMPPD0.05[[1]]) 
dim(myDMPPD0.05$F_to_M)
save(myDMPPD0.05,file = 'GSE111629_champ_myDMPPD0.05.Rdata')
load(file = 'GSE111629_champ_myDMPPD0.05.Rdata')
write.table(myDMPPD0.05, file = "myDMPPD0.05.csv", sep = ",")
...
myDMPPD1 <- champ.DMP(beta = myRefbasePD$CorrectedBeta,
                         pheno = pD$Sex,
                         compare.group = NULL,
                         adjPVal = 1,
                         adjust.method = "BH",
...
head(myDMPPD1[[1]]) 
dim(myDMPPD1$F_to_M)
save(myDMPPD1,file = 'GSE111629_champ_myDMPPD1.Rdata')
load(file = 'GSE111629_champ_myDMPPD1.Rdata')


library(qqman)
qq(myDMPPD1$F_to_M$P.Value)
p_value=myDMPPD1$F_to_M$P.Value
z = qnorm(p_value/ 2)
...
Estimate = myDMPPD1$F_to_M$logFC
StdErr = Estimate/myDMPPD1$F_to_M$t
zvalue = qnorm(myDMPPD1$F_to_M$P.Value / 2)
inflationFactor = median(zvalue^2,na.rm = TRUE) / qchisq(0.5, 1)
print("lambda")
print(inflationFactor)

# genome-wide sig cpgs
sig <- ifelse(myDMPPD1$F_to_M$P.Value < 2.4e-7, 1, 0)
table(sig)
...
class(myDMPPD_bacon)  # "data.frame"
keep <- myDMPPD_bacon$F_to_M.adj.P.Val < 0.05  
myDMPPD0.05_bacon = myDMPPD_bacon[keep,]
write.table(myDMPPD0.05_bacon, file = "myDMPPD0.05_bacon.csv", sep = ",")
...
?champ.DMR
myDMRPD0.05 <- champ.DMR(beta=myRefbasePD$CorrectedBeta,pheno=pD$Sex,method="Bumphunter")
write.table(myDMRPD0.05, file = "myDMRPD0.05.csv", sep = ",")
