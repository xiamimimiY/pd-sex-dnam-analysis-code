...
myLoadHC <-champ.load("GSE145361/idat",
           method="ChAMP",
           methValue="B",
           autoimpute=TRUE,
           filterDetP=TRUE,
           ...
           force=FALSE,
           arraytype="450K")
save(myLoadHC,file = 'GSE145361_champ_myLoadHC777.Rdata')
load(file = 'GSE145361_champ_myLoadHC777.Rdata')
...
champ.QC(beta = myLoadHC$beta,
         ...
         Rplot=TRUE,
         Feature.sel="None",
         resultsDir="./CHAMP_QCimages_777HC/")     #22：10-22：43，

myNormHC <- champ.norm(beta=myLoadHC$beta,
                       rgSet=myLoadHC$rgSet,
                       mset=myLoadHC$mset,
                       resultsDir="./CHAMP_Normalization_777HC/",
                       method="BMIQ",
                       plotBMIQ=FALSE,
                       arraytype="450K",
                       cores=3)
class(myNormHC)   #"matrix" "array" 
dim(myNormHC)  #[1] 404399    777
pD=myLoadHC$pd
pD$Slide<-as.character(as.numeric(pD$Slide))
save(myNormHC,pD,file = 'GSE145361_champ_myNormHC777.Rdata')   
load(file = 'GSE145361_champ_myNormHC777.Rdata')


champ.SVD(beta = myNormHC,
          rgSet=NULL,
          pd=pD,
          RGEffect=FALSE,
          PDFplot=TRUE,
          Rplot=TRUE,
          resultsDir="./CHAMP_SVDimages_777HC/")
...
champ.SVD(beta = myCombatHC %>% as.data.frame(),
          ...
          PDFplot=TRUE,
          Rplot=TRUE,
          resultsDir="./CHAMP_SVDimages_777HC_AftercombatSlide/")
...
myCombatHC <- champ.runCombat(beta=myCombatHC,
                              ...
                              batchname=c("Array"),
                              logitTrans=TRUE)
save(myCombatHC,pD,file = 'GSE145361_champ_myCombatHC777.Rdata')
load(file = 'GSE145361_champ_myCombatHC777.Rdata')
...
champ.SVD(beta = myCombatHC %>% as.data.frame(),
          ...
          PDFplot=TRUE,
          Rplot=TRUE,
          resultsDir="./CHAMP_SVDimages_777HC_AftercombatSlideArray/")
...
class(myNormHC)    #[1] "matrix" "array"
save(myRefbaseHC,pD,file = 'GSE145361_champ_myRefbaseHC777.Rdata')   
load(file = 'GSE145361_champ_myRefbaseHC777.Rdata')


?champ.DMP
myDMPHC0.05_777 <- champ.DMP(beta = myRefbaseHC$CorrectedBeta,
                     pheno = pD$Sex,
                     compare.group = NULL,
                     adjPVal = 0.05,
                     adjust.method = "BH",
                     arraytype = "450K")
head(myDMPHC0.05_777[[1]]) 
dim(myDMPHC0.05_777$M_to_F)
save(myDMPHC0.05_777,file = 'GSE145361_champ_myDMPHC0.05_777.Rdata')
load(file = 'GSE145361_champ_myDMPHC0.05_777.Rdata')
write.table(myDMPHC0.05_777, file = "myDMPHC0.05_777.csv", sep = ",")
...
myDMPHC1_777 <- champ.DMP(beta = myRefbaseHC$CorrectedBeta,
                         pheno = pD$Sex,
                         compare.group = NULL,
                         adjPVal = 1,
                         adjust.method = "BH",
                         arraytype = "450K")
head(myDMPHC1_777[[1]]) 
dim(myDMPHC1_777$M_to_F)
save(myDMPHC1_777,file = 'GSE145361_champ_myDMPHC1_777.Rdata')
load(file = 'GSE145361_champ_myDMPHC1_777.Rdata')


library(qqman)
qq(myDMPHC1_777$M_to_F$P.Value)
p_value=myDMPHC1_777$M_to_F$P.Value
z = qnorm(p_value/ 2)
...
Estimate = myDMPHC1_777$M_to_F$logFC
StdErr = Estimate/myDMPHC1_777$M_to_F$t
zvalue = qnorm(myDMPHC1_777$M_to_F$P.Value / 2)
inflationFactor = median(zvalue^2,na.rm = TRUE) / qchisq(0.5, 1)
print("lambda")
print(inflationFactor)

# genome-wide sig cpgs
sig <- ifelse(myDMPHC1_777$M_to_F$P.Value < 2.4e-7, 1, 0)
table(sig)
...
class(myDMPHC770_bacon)  # "data.frame"
keep <- myDMPHC770_bacon$M_to_F.adj.P.Val < 0.05  
myDMPHC770_0.05_bacon = myDMPHC770_bacon[keep,]
write.table(myDMPHC770_0.05_bacon, file = "myDMPHC770_0.05_bacon.csv", sep = ",")
...
?champ.DMR
myDMRHC0.05_777 <- champ.DMR(beta=myRefbaseHC$CorrectedBeta,pheno=pD$Sex,method="Bumphunter")
write.table(myDMRHC0.05_777, file = "myDMRHC0.05_777.csv", sep = ",")


?DMR.GUI
DMR.GUI(DMR=myDMRHC0.05_777,
       beta=myRefbaseHC$CorrectedBeta,
       pheno=pD$Sex,
       runDMP=TRUE,
       compare.group=NULL,
       arraytype="450K")
